...
```{r}
# Check how many unique FishingZon categories there are
unique(zones_sf$FishingZon)


zones_sf_combined <- zones_sf %>%
  group_by(FishingZon) %>%
  summarize(geometry = st_combine(geometry), .groups = "drop")  # Combine without merging
zones_sf_grouped <- zones_sf_combined %>%
  summarize(geometry = st_union(geometry))  # Now union the combined geometries


# Plot the grouped zones
plot(zones_sf_grouped$geometry)
